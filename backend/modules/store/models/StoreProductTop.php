 ...
 */
class StoreProductTop extends StoreProductSorting
{
    /**
     * @inheritdoc
     */
    public function behaviors()
    {
        return [
            'sort' => [
                ...
                'sortableAttribute' => 'top_position'
            ],
        ];
    }
    ...
    public function search($params)
    {
        $query = static::find()
            ->with(['mainImage'])
            ->where(['is_top_50' => 1])
            ->orWhere(['is_top_50_category' => 1]);

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
            'sort'=> ['defaultOrder' => ['top_position'=> SORT_DESC]]
        ]);

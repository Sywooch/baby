...
class PhoneValidator extends Validator
{
    ...
    public function validateAttribute($model, $attribute)
    {
        ...
        if (!isset($country) && $this->strict) {
            $this->addError($model, $attribute, \Yii::t('frontend', 'You must set country property'));
            return false;
        }
        ...
        try {
            ...
            if ($phoneUtil->isValidNumber($numberProto)) {
                ...
                $this->addError(
                    $model,
                    $attribute,
                    \Yii::t('frontend', 'Phone number does not seem to be a valid phone number')
                );
                return false;
            }
        } catch (NumberParseException $e) {
            $this->addError($model, $attribute, \Yii::t('frontend', 'Unexpected Phone Number Format'));
        } catch (Exception $e) {
            $this->addError($model, $attribute, \Yii::t('frontend', 'Unexpected Phone Number Format or Country Code'));
        }
    }
}
